...
try {
    ...
    if (init('action') == 'sendToBdd') 	{
		config::save('FREEBOX_SERVER_TRACK_ID', init('track_id'),'Neuronal');
		config::save('FREEBOX_SERVER_APP_TOKEN', init('app_token'),'Neuronal');
		ajax::success(true);
    }
	if (init('action') == 'connect') 	{
		ajax::success(Neuronal::track_id());
    }	
	if (init('action') == 'ask_track_authorization') 	{
		ajax::success(Neuronal::ask_track_authorization());
    }
	if (init('action') == 'SearchReseau') 	{
		ajax::success(Neuronal::freeboxPlayerPing());
    }
	...
	if (init('action') == 'WakeOnLAN')	{
		...
		if(is_object($Commande)){
			...
			ajax::success(Neuronal::WakeOnLAN($Mac));
		}
		ajax::success(false);
    }	
